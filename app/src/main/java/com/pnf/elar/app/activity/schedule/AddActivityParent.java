...
public class AddActivityParent extends AppCompatActivity {
    ...
    public void initView() {
        try {
            ...
            dialogLoading = new MyCustomProgressDialog(AddActivityParent.this);
           ...
           startDateText = (TextView) findViewById(R.id.startDateText);
            ...
            childrensAdapter=new ChildrensAdapter(context,childrensList);
            LinearLayoutManager layoutManager = new LinearLayoutManager(context);
            childrenRv.setLayoutManager(layoutManager);
            childrenRv.setAdapter(childrensAdapter);
            ...
            case R.id.startDateText:
                ...
                try {
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                        ...
                        new DatePickerDialog(AddActivityParent.this, startDateDg, startCalendar
                                .get(Calendar.YEAR), startCalendar.get(Calendar.MONTH),
...
//                try {
//                    new DatePickerDialog(AddActivityParent.this, endDateDg, enddateCalendar
//                            .get(Calendar.YEAR), enddateCalendar.get(Calendar.MONTH),
//                            enddateCalendar.get(Calendar.DAY_OF_MONTH)).show();
//                } catch (Exception e) {
//                    e.printStackTrace();
...
*/
                    ...
                    startMinute = startCalendar.get(Calendar.MINUTE);
                    startTimeText.setText(startHour + ":" + startMinute);
                    startTimeDg = new TimePickerDialog(AddActivityParent.this,
                            new TimePickerDialog.OnTimeSetListener() {
...
*/
            ...
            case R.id.endTimeText:
                if (Build.VERSION.SDK_INT > Build.VERSION_CODES.M) {
                    ...
                    endTimeDg = new TimePickerDialog(AddActivityParent.this,
                            new TimePickerDialog.OnTimeSetListener() {
...
*/
    ...
    private void callChildrenList() {
        try {
            ...
            if (SmartClassUtil.isNetworkAvailable(context)) {
                ...
                Call<ChildrensBo> response = retrofit.create(API.class).viewParentUser(body);
                response.enqueue(new Callback<ChildrensBo>() {
                    @Override
                    public void onResponse(Call<ChildrensBo> call, Response<ChildrensBo> response) {
                        dialogLoading.dismiss();
                        if (response.body() != null) {
                            ChildrensBo childrensBo = response.body();
                            if (childrensBo.getStatus().equalsIgnoreCase(Const.Params.TRUE)) {
                                if(!childrensBo.getChildren().isEmpty()&&childrensBo.getChildren()!=null) {
                                    for (int i = 0; i < childrensBo.getChildren().size(); i++) {
                                        ChildrensBo.ChildrenBean childrenBean = childrensBo.getChildren().get(i);
                                        childrenBean.setSelected(false);
                                        childrensList.add(childrenBean);
                                    }
                                }
                                childrensAdapter.notifyDataSetChanged();
                            } else {
                                SmartClassUtil.showToast(context, childrensBo.getMessage());
                            }
                        }
                    }

                    @Override
                    public void onFailure(Call<ChildrensBo> call, Throwable t) {
                        dialogLoading.dismiss();
                    }
                });
    ...
    public void checkValidation()
    {
        ...
        for(int i=0;i<childrensList.size();i++)
        {
            if(childrensList.get(i).isSelected())
            {
                childList.put(childrensList.get(i).getId());
            }
        }
       ...
       /* if (courseId.length() == 0 || ("0").equalsIgnoreCase(courseId)) {
        ...
        {
            if (language.equals("en")) {
                SmartClassUtil.showToast(context, "please select child");
            } else {
                SmartClassUtil.showToast(context, "välj barn");
            }
        }
        ...
        try {
            ...
            if (SmartClassUtil.isNetworkAvailable(context)) {
                ...
                Retrofit retrofit = new Retrofit.Builder()
                        ...
                        .build();
                RequestBody body = RequestBody.create(MediaType.parse("application/json; charset=utf-8"), reqJsonObj.toString());
                Call<String> response = retrofit.create(API.class).addactivityforParent(body);
